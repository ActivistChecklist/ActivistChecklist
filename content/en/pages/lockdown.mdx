...
answerCapsule: >
  Lockdown Mode (iPhone) and Advanced Protection (Android) make your
  device much harder to attack with spyware, but they break some normal
  features. On iPhone, no clickable links in iMessage, blocked message
  attachments, FaceTime from unknowns blocked, no web fonts, no
  auto-join for insecure Wi-Fi. On Android, security key required for new sign-ins,
  no app sideloading, USB locked. For high-risk users, the protection
  is usually worth it.
