---
title: Consider getting a secondary or burner phone for activist work
titleBadges: []
---
If it fits your workflow, consider setting up a second phone or computer just for your higher-risk work.

Check our [Secondary Phone Checklist](/secondary/) for more on this.

If you do this, make sure you still take security precautions on all devices. Even though only one device has sensitive communications/data on it, both devices could be a target for spyware since they reveal your location and other personal information an attacker might want.
You might consider getting a phone that supports **[GrapheneOS](https://grapheneos.org/)**. It is security-focused and privacy-focused version of Android that runs on Google Pixel phones.