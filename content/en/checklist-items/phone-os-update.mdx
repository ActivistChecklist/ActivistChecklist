---
title: Keep your phone updated to the latest version of its operating system
preview: |
  Protects you from hackers, spyware, and passcode cracking tools.
do: |
  Install updates as soon as you're prompted
dont: |
  Keep pushing the "update later" button
titleBadges: []
firstPublished: 2026-01-28
lastUpdated: 2026-01-31
---
The cops have tools that rely on weaknesses in your operating system that get fixed in the latest versions.

Every time a new update comes out, that usually means there are new public vulnerabilities on your device. Think of it as a ticking clock, where hackers now know a new way in to any out-of-date devices.

<HowTo title="How to update your phone's operating system">
  <MenuPath>*iPhone:* Go to **Settings** > **General** > **Software Update** > If you see an option to update, tap **Install now**</MenuPath>

  <MenuPath>*Android:* **Settings** > **System** > **System update** (or **Software update**) > Tap **Check for update** or **Download and install** if an update is available</MenuPath>
</HowTo>

If your operating system is not up-to-date, many of the other recommendations in this guide will not protect your data.
