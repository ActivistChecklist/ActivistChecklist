---
title: Action Research & Scouting
seoDescription: Research an action target without leaving a trail. Use Tor Browser for online research and a phone with minimal data if you have to scout in person.
answerCapsule: >
  For sensitive online research, use Tor Browser. It hides your IP and
  makes it much harder (though not impossible) for sites to link what
  you're looking at back to you. Don't sign in to any accounts while
  using it. For in-person
  scouting, leave your phone home if you can. If you need it, use
  Organic Maps offline and the Signal in-app camera instead of your
  regular camera so photos go to encrypted threads.
estimatedTime: 15 minutes
excerpt: >
  For anyone doing sensitive online research or in-person scouting who wants to
  protect their identity.
firstPublished: 2025-01-09
lastUpdated: 2025-09-09
---
<Section title="Use Tor Browser for sensitive research" slug="tor">
  When preparing for a higher-risk protests, it's very helpful to research the location or target online. This is a particularly important time to protect your identity so you don't become a target for more state surveillance.

  [**Install Tor Browser**](https://www.torproject.org/download/) **and use it for your sensitive research.**

  Websites have advanced ways of tracking your browsing, even if you’re not logged in. Your IP address can be used to identify you.

  Tor encrypts your traffic, relays it through multiple computers, and goes to great lengths to prevent online tracking in order to help you remain more anonymous online.

  **Example use cases:** These are a few examples of situations where you may want to remain anonymous to protect yourself, your plans, and your comrades from being targeted by the state and corporations.

  * Researching the location of an action on Google Maps or Google Street View.
  * Researching who is on the board of a corporation you’re running a campaign against
  * Researching equipment to protect against tear gas

  <Alert type="warning">
    **Do not sign in to any accounts** while using Tor! This can expose your identity.
  </Alert>

  **Advanced:** Tor Browser can still leave behind small traces. Or if you have spyware on your computer, your activity can be monitored. To get the most complete anonymity possible, use the [Tails](https://tails.net/) operating system. It is an "ephemeral" operating system that uses Tor and never writes any information to a disk. You install it onto a USB stick and boot a "fresh" installation every time.
</Section>

<Section title="Scouting for a direct action">
  Most direct actions include a phase where some people scout the location to understand the area and how it's being used (security guards, surveillance cameras, etc.)

  **Ideally you would not bring any phones** and not take any photos. That said, sometimes the situation requires it.

  <ChecklistItemGroup>
    <ChecklistItem slug="organic-maps" />

    <ChecklistItem slug="signal-camera" />
  </ChecklistItemGroup>
</Section>
