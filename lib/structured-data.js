...
/**
 * Parse "45 minutes", "1 hour", "30-60 minutes", "3 hours", "1 hour 30
 * minutes", "1 hour to start, 4 hours to finish", etc. into an ISO 8601
 * duration like PT45M / PT1H / PT1H30M.
 *
 * Two different intents to disambiguate:
 *   - Additive ("1 hour 30 minutes") → sum the pairs → PT1H30M.
 *   - Range / alternative ("30-60 minutes", "1 hour to start, 4 hours to
 *     finish") → pick the upper bound.
 *
 * Heuristic: if the string contains a `-` or the word `to`, treat it as a
 * range. Otherwise sum. Unit preservation: a single-unit input like
 * "60 minutes" stays as PT60M (not normalized to PT1H) to honor the
 * author's choice of unit.
 */
export function parseEstimatedTimeToIso(value) {
  if (!value) return null;
  const s = String(value).toLowerCase();

  // Collect every "<n> <unit>" pair.
  const pairRe = /(\d+(?:\.\d+)?)\s*(hours?|hrs?|hr|minutes?|mins?|min)\b/g;
  const pairs = [];
  let m;
  while ((m = pairRe.exec(s)) !== null) {
    const n = parseFloat(m[1]);
    if (!Number.isFinite(n) || n <= 0) continue;
    const isHour = /^h/.test(m[2]);
    pairs.push({ n, isHour, minutes: isHour ? n * 60 : n });
  }

  if (pairs.length === 0) {
    // No "<n> <unit>" pairs found. Last-ditch: a bare number with a unit
    // keyword nearby (e.g. "minutes" elsewhere in the string).
  ...
  // Range / alternative: take the pair with the largest duration.
  if (/-|\bto\b/.test(s)) {
    const max = pairs.reduce((a, b) => (b.minutes > a.minutes ? b : a));
    return max.isHour
      ? `PT${Math.round(max.n)}H`
      : `PT${Math.round(max.n)}M`;
  }

  // Additive: sum all pairs. Preserve single-unit framing (PT60M, not PT1H)
  // when the author used only one unit.
  let totalMinutes = 0;
  let sawHour = false;
  let sawMinute = false;
  for (const p of pairs) {
    totalMinutes += Math.round(p.minutes);
    if (p.isHour) sawHour = true;
    else sawMinute = true;
  }
  if (totalMinutes <= 0) return null;
  if (sawHour && !sawMinute) return `PT${Math.round(totalMinutes / 60)}H`;
  if (sawMinute && !sawHour) return `PT${totalMinutes}M`;
  const h = Math.floor(totalMinutes / 60);
  const m2 = totalMinutes % 60;
  if (h > 0 && m2 > 0) return `PT${h}H${m2}M`;
...
export function buildArticle({ baseUrl, locale, slug, frontmatter = {}, imageUrl, type = 'Article' }) {
  ...
  // seoTitle wins when both are set: it's the SEO-facing variant.
  const headline = frontmatter.seoTitle || frontmatter.title || '';
