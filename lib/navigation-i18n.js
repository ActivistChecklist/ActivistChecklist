/**
 * Wraps next-intl `t` so missing keys fall back to the English string from the
 * navigation config.
 *
 * next-intl does NOT throw on a missing message: it reports MISSING_MESSAGE to
 * its onError handler and returns the key path itself as the rendered string.
 * So a bare try/catch never fires, and the raw key ("navItems.vpn.title") would
 * be shown to readers whenever a locale is missing a nav string — which happens
 * routinely between an English release and the Crowdin sync. Ask `t.has()` first
 * so the fallback actually applies and no console error is logged.
 *
 * The try/catch is kept for genuine throws (e.g. message formatting errors), and
 * `t.has` is feature-detected so a plain `(key) => string` still works.
...
const ITEM_TRANSLATION_KEYS = {
  ...
  vpn: {
    title: 'navItems.vpn.title',
    label: 'navItems.vpn.label',
  },
  movies: {
  ...
  updates: {
    title: 'navItems.updates.title',
    label: 'navItems.updates.label',
  },
  resources: {
...
export function translateNavigationItem(item, translateText) {
  ...
  const translatedItem = {
    ...
    // Footer and other UIs use `title || label`; top-level nav items (home, news)
    // only had label translated, so align title with the same string.
    title: topLevelKey
      ? translateText(topLevelKey, item.title ?? item.label)
      : merged.title,
