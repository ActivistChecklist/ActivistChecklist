...
import { isReleaseOsCapSuspect } from './os-cap-audit.js';
...
export function buildOsCheckOptions(snapshot, product, release) {
  ...
  const range = parseOsRange(release.supportedOsRange);
  // When the ceiling is stale (see os-cap-audit.js) truncating to it would hide the
  // major the user is actually running — they physically could not tell us they were
  // on iOS 27 while the snapshot still capped every iPhone at 26. Offering a major a
  // device genuinely can't run is a self-correcting mistake (the user just doesn't
  // pick it); hiding the one they're on is not.
  const capIsSuspect = isReleaseOsCapSuspect(snapshot, product, release);
  ...
  const options = osProduct.releases
    ...
    .filter((r) => !range || capIsSuspect || parseFloat(r.id) <= range.max)
    .map((r) => ({
...
export function buildAppleSupportEstimate(product, release, now = new Date()) {
  ...
  // The whole estimate is "launch date plus a support window", so an inferred
  // launch date would compound into a remaining-years figure we'd print as a
  // number. Better to show no estimate than a confident wrong one.
  if (release.releaseDateIsEstimate) return null;
...
export function buildDeviceMaxOsWarning(snapshot, product, release, reminder, now = new Date()) {
  ...
  // A stale ceiling (see os-cap-audit.js) produces exactly this shape: device max one
  // major below the family latest. Saying nothing is strictly better than telling
  // someone on current hardware that it has hit its OS ceiling and to plan a hardware
  // upgrade — the advice is expensive, hard to walk back, and wrong.
  if (isReleaseOsCapSuspect(snapshot, product, release)) return null;
