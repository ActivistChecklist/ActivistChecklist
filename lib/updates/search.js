...
// Recency is more than a tiebreaker but less than a primary signal — a precise
// text match (fuse score ~0) should still beat a newer but fuzzy match (fuse
// score > 0.2), but among matches with SIMILAR fuse scores (e.g. several
// labels that all contain the query as a substring, where the only difference
// is label length), the newer one should sort first. Bumped from 0.05 → 0.15
// to give "macbook pro" / "galaxy" / "ipad" queries newest-first ordering
// without inverting precise vs fuzzy match priority.
  ...
  for (const product of snapshot.products) {
    for (const release of product.releases) {
      ...
      rows.push({
        ...
        releaseDateIsEstimate: release.releaseDateIsEstimate === true,
        recencyScore,
      });
    }
  }
