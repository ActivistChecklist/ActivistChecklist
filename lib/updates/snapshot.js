...
/**
 * Returns the input only if it parses as an http(s) URL. Defends against a malicious
 * or compromised snapshot supplying javascript: / data: / etc. URLs that would XSS via
 * <a href> in the result card.
 */
...
export function normalizeRelease(raw) {
  return {
    ...
    // True when releaseDate was inferred from the oldest OS the device can run
    // rather than read off the marketing name (see sofa-macos.js). Fine to sort
    // and rank by; never present it to the user as the launch date.
    releaseDateIsEstimate: raw.releaseDateIsEstimate === true,
    ...
    latestVersionLink: safeHttpUrl(raw.latestVersionLink),
...
export function normalizeProduct(raw) {
  return {
    ...
    endoflifeUrl: safeHttpUrl(raw.endoflifeUrl),
 ...
 *
 * No signal is accepted: aborting one caller's signal must not cancel the shared fetch and
 * leave other concurrent callers permanently stuck in loading.
 */
export async function loadSnapshot() {
  ...
  inflight = (async () => {
    try {
      // 'no-cache' makes the browser send a conditional GET (If-None-Match) on
      // every load. nginx/Apache return 304 Not Modified when the file's ETag
      // hasn't changed (the build script skips writes when content is identical,
      // so the ETag stays stable across cron runs that didn't change anything).
      // The 304 round-trip is ~200 bytes; the full ~50 KB gzipped body is only
      // re-fetched when the snapshot actually changed upstream.
